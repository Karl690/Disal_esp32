...
void ui_pcnt_event_click_cb(lv_event_t* e)
{
	...
	ui_helpers_button_color(ui_pcnt.enabled, systemconfig.pcnt.enabled == 1? 0x00ff00 : 0xff0000, UI_FOREGROUND_COLOR);
	ui_helpers_button_text(ui_pcnt.enabled, systemconfig.pcnt.enabled == 1 ? "ON" : "OFF");
}
...
void ui_pcnt_init(void)
{
	...
	lv_obj_set_pos(ui_pcnt.focus, 35, y + 5);
	...
	y += step;
	obj = ui_helpers_create_button(ui_pcnt_screen, systemconfig.pcnt.enabled == 1? "ON": "OFF", 100, 40, 5, &lv_font_montserrat_14, ui_pcnt_event_click_cb, NULL);
	ui_helpers_button_color(obj, systemconfig.pcnt.enabled == 1? 0x00ff00: 0xff0000, UI_FOREGROUND_COLOR);
