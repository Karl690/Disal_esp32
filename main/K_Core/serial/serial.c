#ifdef CONFIG_APP_USE_SERIAL
#include "configure.h"
#include "main.h"

#include "serial.h"
#include <stdio.h>
#include <sys/fcntl.h>
#include <sys/errno.h>
#include <sys/unistd.h>
#include <sys/select.h>
#include "L_Core/bluetooth/ble.h"
#include "K_Core/communication/communication.h"
#include "L_Core/ui/ui-comm.h"
#include "K_Core/simple/simple.h"
#include "K_Core/secs/secs.h"
int serial_uart_fd = -1;


uint8_t serial_uart1_rx_buffer[RX_BUF_SIZE] = {0};
uint8_t serial_uart1_tx_buffer[TX_BUF_SIZE] = {0};

uint8_t serial_uart2_rx_buffer[RX_BUF_SIZE] = {0};
uint8_t serial_uart2_tx_buffer[TX_BUF_SIZE] = {0};

COMPORT ComUart1, ComUart2;
COMPORT* MasterComPort = &ComUart1;
uint8_t serial_uart1_last_read_buffer[256] = {0};
uint8_t serial_uart2_last_read_buffer[256] = {0};

bool serial_valid_pins(uint8_t pin)
{
	if ((pin >= 10 && pin <= 14) || pin == 21) return true;
	return false;
}
void serial_uart_init(uart_port_t port, int tx_pin, int rx_pin, int baud, int rts_pin, int cts_pin, bool is485)
{
	if (!serial_valid_pins(tx_pin)) return;
	if (!serial_valid_pins(rx_pin)) return;
	const uart_config_t uart_config = {
		.baud_rate = baud,
		.data_bits = UART_DATA_8_BITS,
		.parity = UART_PARITY_DISABLE,
		.stop_bits = UART_STOP_BITS_1,
		.flow_ctrl = UART_HW_FLOWCTRL_DISABLE,
		.source_clk = UART_SCLK_APB,
	};
	// We won't use a buffer for sending data.
	uart_driver_install(port, RX_BUF_SIZE * 2, 0, 0, NULL, 0);
	uart_param_config(port, &uart_config);
	uart_set_pin(port, tx_pin, rx_pin, rts_pin, cts_pin);
	if (is485)
	{
		// Set RS485 half duplex mode	
		ESP_ERROR_CHECK(uart_set_mode(port, UART_MODE_RS485_HALF_DUPLEX));
	}
	// Enable UART DMA
	uart_set_mode(port, UART_MODE_UART);
}


void serial_uart_update_config(uart_port_t port, int tx_pin, int rx_pin, int baud)
{
	const uart_config_t uart_config = {
		.baud_rate = baud,
		.data_bits = UART_DATA_8_BITS,
		.parity = UART_PARITY_DISABLE,
		.stop_bits = UART_STOP_BITS_1,
		.flow_ctrl = UART_HW_FLOWCTRL_DISABLE,
		//.source_clk = UART_SCLK_APB,
	};
	uart_param_config(port, &uart_config);
	uart_set_pin(port, tx_pin, rx_pin, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
}
bool serial_uart_write_byte(COMPORT* comport, char byte)
{
	if (uart_write_bytes((uart_port_t)comport->uart_id, &byte, 1) != 1) {
		ESP_LOGE(TAG, "Send data critical failure.");
		// add your code to handle sending failure here
		return false;
	}
	comport->TxIndicator = 3;
	comport->NumberOfCharactersSent++;
	return true;
}

void serial_uart1_read_task(void* param)
{
	uint8_t buffer[RX_BUF_SIZE];
	while (1)
	{
		if (!IsInitialized) {
			vTaskDelay(10);
			continue;
		}
		int len = uart_read_bytes(UART_NUM_1, buffer, RX_BUF_SIZE, 10 / portTICK_PERIOD_MS); //(100 / portTICK_PERIOD_MS));	
		if (len > 0) {
			ESP_LOGI(TAG, "uart1- read -data: %d", len);
			ComUart1.NumberOfCharactersReceived += len;
			ComUart1.RxIndicator = INDICATER_COUNTDOWN;
			communication_add_buffer_to_serial_buffer(&ComUart1.RxBuffer, buffer, len);
		}
	}
}
void serial_uart2_read_task(void* param)
{
	uint8_t buffer[RX_BUF_SIZE /2]; // 2kbytes
	while (1)
	{
		if (!IsInitialized) {
			vTaskDelay(10);
			continue;
		}
		int len = uart_read_bytes(UART_NUM_2, buffer, RX_BUF_SIZE /2, 10 / portTICK_PERIOD_MS); //(10 / portTICK_PERIOD_MS)
		if (len > 0)
		{
			ESP_LOGI(TAG, "uart1- read -data: %d", len);
			ComUart2.NumberOfCharactersReceived += len;
			ComUart2.RxIndicator = INDICATER_COUNTDOWN;
			communication_add_buffer_to_serial_buffer(&ComUart2.RxBuffer, buffer, len);	
		}
	}
}

uint8_t serial_buffer1[RX_BUF_SIZE] = {0}; // 2kbytes
uint8_t serial_buffer2[RX_BUF_SIZE] = {0}; // 2kbytes
void serial_uart1_read() {
	int len = uart_read_bytes(UART_NUM_1, serial_buffer1, RX_BUF_SIZE, 1 / portTICK_PERIOD_MS); //(100 / portTICK_PERIOD_MS));	
	if (len > 0) {
		ESP_LOGI(TAG, "uart1- read -data: %d", len);
		ComUart1.NumberOfCharactersReceived += len;
		ComUart1.RxIndicator = INDICATER_COUNTDOWN;
		communication_add_buffer_to_serial_buffer(&ComUart1.RxBuffer, serial_buffer1, len);
	}
}

void serial_uart2_read() {
	int len = uart_read_bytes(UART_NUM_2, serial_buffer2, RX_BUF_SIZE, 1 / portTICK_PERIOD_MS); //(100 / portTICK_PERIOD_MS));	
	if (len > 0) {
		ESP_LOGI(TAG, "uart2- read -data: %d", len);
		ComUart1.NumberOfCharactersReceived += len;
		ComUart1.RxIndicator = INDICATER_COUNTDOWN;
		communication_add_buffer_to_serial_buffer(&ComUart2.RxBuffer, serial_buffer2, len);
	}
}

void serial_init()
{
	// initialize uart devices
	serial_uart_init(UART_NUM_1, systemconfig.serial1.tx_pin, systemconfig.serial1.rx_pin, systemconfig.serial1.baud, SERIAL_UART1_RTS_PIN, SERIAL_UART1_CTS_PIN, false);
	communication_buffers_serial_init(UART_NUM_1, &ComUart1, serial_uart1_rx_buffer, serial_uart1_tx_buffer);
	
	if (systemconfig.enableSerial2 && (systemconfig.serial1.tx_pin != systemconfig.serial2.tx_pin 
		&& systemconfig.serial1.rx_pin != systemconfig.serial2.rx_pin))
	{
		serial_uart_init(UART_NUM_2, systemconfig.serial2.tx_pin, systemconfig.serial2.rx_pin, systemconfig.serial2.baud, SERIAL_UART2_RTS_PIN, SERIAL_UART2_CTS_PIN, false);	
		communication_buffers_serial_init(UART_NUM_2, &ComUart2, serial_uart2_rx_buffer, serial_uart2_tx_buffer);
	} else {
		systemconfig.enableSerial2 = false;
	}
	
	// xTaskCreatePinnedToCore(serial_uart1_read_task, "uart_thread1", RX_BUF_SIZE * 1, NULL, 10, NULL, 1);
	// xTaskCreatePinnedToCore(serial_uart2_read_task, "uart_thread2", RX_BUF_SIZE * 1, NULL, 10, NULL, 1);
}
#endif