...
void communication_buffers_serial_init(uint8_t UartIndex, COMPORT* ComPort, uint8_t* RxBuffer, uint8_t* TxBuffer)
{	
...
{
	if (systemconfig.enableSerial2 == false) return;
...
void communication_tx_commandline(COMPORT* comport, char* commandline)
{
	...
	if (systemconfig.enableSerial2 == false && comport->uart_id == UART_NUM_2) return;
