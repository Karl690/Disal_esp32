    ...
    case KNOB_PHASE_A:
        if (knob->encoder_b_change) {
            ...
            if (knob->default_direction) {
                ESP_LOGI(TAG, "KNOB_LEFT event KNOB_PHASE_A 1");
                knob->count_value--;
                ...
                if(knob->cb[KNOB_RIGHT]) {
                    knob->cb[KNOB_RIGHT](knob, knob->usr_data[KNOB_RIGHT]);
                }
    ...
    case KNOB_PHASE_B:
        if (knob->encoder_a_change) {
            ...
            if (knob->default_direction) {
                ...
                knob->count_value--;
                knob->event = KNOB_LEFT;
                if(knob->cb[KNOB_LEFT]) {
                    knob->cb[KNOB_LEFT](knob, knob->usr_data[KNOB_LEFT]);
                }
                // CALL_EVENT_CB(KNOB_LEFT);
                if (knob->count_value <= LOW_LIMIT) {
                    knob->event = KNOB_L_LIM;
